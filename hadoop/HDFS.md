...
1. 协议: `TCP协议`
2. 心跳: DN每隔`3秒`向NN发送心跳信息，表明DN处于存活状态;
3. 块报告: DN每隔`10秒`向NN发送数据块报告，用于NN更新元数据；
...
1. gzip: 文件压缩后小于块大小，或者小于分片大小;
2. lzo: 单个文件较大，充分发挥其压缩/解压缩快的特点;
3. snappy: MR作业中间map输出文件较大时，用作中间输出文件的压缩；或者作为一个MR作业的输出和另一个MR作业的输入;
4. bzip2: 对压缩速度要求低，本机磁盘空间有限时常用此压缩;其次，其解压缩速率也慢，常用于长期备份且不会经常被使用的数据;
...
## 12. [HDFA HA](http://hadoop.apache.org/docs/r2.7.6/hadoop-project-dist/hadoop-hdfs/HDFSHighAvailabilityWithQJM.html)
